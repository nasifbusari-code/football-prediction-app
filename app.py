...
def make_prediction(data_dict, match_info, match_id):
    ...
    if zero_count in [4, 5, 6] and meta_probs[0] > meta_probs[1]:  # Adjusted for 3 matches
        ...
        if over_1_5_odds and 1.05 <= over_1_5_odds <= 1.30:  # Relaxed range
            ...
            reason = f"Meta-Model Over 1.5 Probability ({meta_over_prob:.1f}%) meets 60% threshold, but Over 1.5 odds ({over_1_5_odds:.2f if over_1_5_odds is not None else 'N/A'}) are unavailable or outside 1.05-1.30 range."
        ...
        if under_3_5_odds and 1.05 <= under_3_5_odds <= 1.30:  # Relaxed range
            ...
            reason = f"Meta-Model Under 3.5 Probability ({meta_under_prob:.1f}%) meets or exceeds 60% threshold, and Under 3.5 odds ({under_3_5_odds:.2f if under_3_5_odds is not None else 'N/A'}) are within 1.05-1.30 range."
        else:
            recommendation = "Under 3.5 (No Odds)"
            reason = f"Meta-Model Under 3.5 Probability ({meta_under_prob:.1f}%) meets 60% threshold, but Under 3.5 odds ({under_3_5_odds:.2f if under_3_5_odds is not None else 'N/A'}) are unavailable or outside 1.05-1.30 range."
    else:
        ...
        with open('predictions.txt', 'w', encoding='utf-8') as f:
            ...
            for result in results:
                output = (f"\nMatch: {result['Match']}\n"
                          ...
                          f"Over 1.5 Odds: {result['Over1_5Odds']:.2f if result['Over1_5Odds'] is not None else 'N/A'}\n"
                          f"Under 3.5 Odds: {result['Under3_5Odds']:.2f if result['Under3_5Odds'] is not None else 'N/A'}\n"
