...
# Flask App Configuration (unchanged)
app = Flask(__name__)
...
# Routes for Database Initialization (unchanged)
...
def favour_v6_confidence(row, HomeGoalList, HomeConcededList, AwayGoalList, AwayConcededList, HomeBTTS, AwayBTTS, poisson_prob):
    ...
    avg_conceded_both = (np.sum(HomeConcededList) + np.sum(AwayConcededList)) / 10
    ...
    total_goals_per_match = np.sum(all_goals) / 10
...
def make_prediction(data_dict, match_info, match_id):
    ...
    odds = asyncio.run(fetch_odds_async(ClientSession(), match_id, asyncio.Semaphore(10)))
...
async def main_async(date_from=None):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # Fetch match data and odds concurrently
        match_data_tasks = []
        odds_tasks = []
        for match in all_matches:
        ...
        # Execute match data and odds fetching concurrently
...
# Wrapper for main function to run async
...
# Routes (unchanged except for home)
@app.route('/')
def home():
    try:
